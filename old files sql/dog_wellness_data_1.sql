...
Alter table event_info add duration time not null after end_time;
Alter table event_info add activity_id tinyint not null after id;
alter table event_info add foreign key(activity_id) references activity(id);
...
alter table dog modify weight enum("large","small","medium");
alter table dog rename column weight to size;
